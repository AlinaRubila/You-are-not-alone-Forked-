...
screen navigation():
    ...
    vbox:
        ...
        spacing gui.navigation_spacing
            ...
            #imagebutton auto "gui/mm_start_%s.png" xpos 150 ypos 390 focus_mask True action Start()
...
## Экран главного меню и галлереи#########################################################

screen main_menu():

    tag menu

    add "images/mm/bg_class_1.png"
    imagebutton auto "gui/mm/mm_quit_%s.png" xpos 1638 ypos 255 focus_mask True action Quit(confirm=not main_menu) hovered [ Play("sound", "audio/zvuk11.ogg") ]
    add "images/mm/bg_class_logo.png"
    add "images/mm/bg_class_2.png"
    imagebutton auto "gui/mm/mm_preferences_%s.png" xpos 490 ypos 480 focus_mask True action ShowMenu("preferences") hovered [ Play("sound", "audio/zvuk11.ogg") ]
    add "images/mm/bg_class_4.png"
    imagebutton auto "gui/mm/mm_gallery_%s.png" xpos 340 ypos -20 focus_mask True action ShowMenu("gallery") hovered [ Play("sound", "audio/zvuk11.ogg") ]
    add "images/mm/bg_class_6.png"
...
screen zhenya:
    ...
    hbox:
        ...
        if persistent.jenya_meet and (persistent.ending1 or persistent.ending2):
            text "{font=Assya Font Official.otf}{size=+5}Знакомство с Женей обещает много веселья. И я не просто думаю об этом. Я это чувствую.{/size}{/font}"
        elif persistent.jenya_meet:
            text "{font=Assya Font Official.otf}{size=+5}Знакомство с Женей обещает много веселья{/size}{/font}"
    hbox:
        spacing 0
        xsize 600
        ysize 20
        xalign 0.25 yalign 0.34
        if persistent.jenya_dreamcontinue:
            text "{font=Assya Font Official.otf}{size=+5}27.01 Мне показалось, что я смог разглядеть девушку из своих снов во тьме. Интересно, почему она носила летнюю одежду зимой?{/size}{/font}"
    hbox:
        spacing 0
        xsize 600
        ysize 50
        ...
        if persistent.jenya_endchap1:
            text "{font=Assya Font Official.otf}{size=+5}Может, пригласить Женю погулять на выходных? Она очень милая. Заодно разузнаю побольше про одногруппников.{/size}{/font}"
    hbox:
        spacing 0
        xsize 600
        ysize 100
        xalign 0.25 yalign 0.64
        if persistent.jenya_endchap2:
            text "{font=Assya Font Official.otf}{size=+5}19.02 Я заметил, что иногда амулет Жени становится тёплым сам по себе, особенно ночью. Неужели он так реагирует на призрака?{/size}{/font}"
    hbox:
        spacing 0
        xsize 595
        ysize 50
        xalign 0.25 yalign 0.78
    ...
    hbox:
        ...
        xsize 600
        ysize 100
        xalign 0.9 yalign 0.78
        if persistent.jenya_frombin:
            text "{image=gui/diary_pictures/cj_portret.png}"
    hbox:
        spacing 0
        xsize 600
        ysize 100
...
screen timur:
    ...
    hbox:
        ...
        ysize 40
        xalign 0.25 yalign 0.1
        if persistent.first_note and persistent.timur_dreamcontinue:
            text "{font=Assya Font Official.otf}{size=+5}26.01 Сон какой-то странный приснился, слишком реальный... Может, это реакция на стресс {i}и, возможно, немного хаоса{/i}{/size}{/font}?"
        elif persistent.first_note:
            text "{font=Assya Font Official.otf}{size=+5}26.01 Сон какой-то странный приснился, слишком реальный... Может, это реакция на стресс?{/size}{/font}"
    ...
    hbox:
        ...
        if persistent.timur_first_note:
            text "{font=Assya Font Official.otf}{size=+5}Условия в общежитии превзошли мои ожидания в лучшую сторону. Буду осваиваться и ждать первого таракана, который упадёт мне на голову.{/size}{/font}"

    hbox:
        spacing 0
        xsize 600
        ysize 20
        xalign 0.25 yalign 0.41
        if persistent.timur_dreamcontinue:
            text "{font=Assya Font Official.otf}{size=+5}27.01 Сегодня я опять видел её. Незнакомку. Но почему-то мне показалось, что я знаю её имя. Но не могу вспомнить.{/size}{/font}"

    hbox:
        spacing 0
        xsize 600
        ysize 20
        xalign 0.25 yalign 0.54
        if persistent.timur_argue:
            text "{font=Assya Font Official.otf}{size=+5}Ну и сосед мне попался. Неужели так сложно согласиться на этот плакат? Я точно буду переселяться.{/size}{/font}"

    hbox:
        spacing 0
        xsize 600
        ysize 100
        xalign 0.25 yalign 0.66
        if persistent.timur_feeling:
            text "{font=Assya Font Official.otf}{size=+5}17.02 Меня преследует ощущение, что это был вещий сон. Но вдруг настоящая опасность ещё впереди?{/size}{/font}"
    hbox:
        spacing 0
        xsize 600
        ysize 20
    ...
    hbox:
        ...
        if persistent.timur_help:
            text "{font=Assya Font Official.otf}{size=+5}Я рад, что смог помочь Тимуру избежать травмы. Всё-таки сон и вправду был вещим.{/size}{/font}"
    ...
    hbox:
        ...
        if persistent.timur_fun:
            text "{font=Assya Font Official.otf}{size=+5}Оказывается, Тимур интересный собеседник. Просто… ему нужна поддержка. Он должен знать, что он не один.{/size}{/font}"

    hbox:
        spacing 0
        xsize 600
        ysize 50
        xalign 0.78 yalign 0.8
        if persistent.timur_bad and not(persistent.timur_good):
            text "{font=Assya Font Official.otf}{size=+5}19.02 И как я должен был справляться без Тимура? Я и так сделал всё, что мог. Ещё и обои эти – кто на них дырки оставил?{/size}{/font}"
        elif persistent.timur_good:
            text "{font=Assya Font Official.otf}{size=+5}19.02 Благодаря советам Тимура работать было легче. Я, конечно, устал за день, но, главное, всё сделал правильно. Кстати, странно, что в углу на обоях нарисован какой-то смайлик.{/size}{/font}"
...
screen gallery():
    ...
    grid 4 3 xpos 70 ypos 100:
        xfill True
        yfill True

        add g.make_button("poster1", "p_1", xalign=1.3, yalign=0.5, hover_border="images/posters/hover.png")
        add g.make_button("poster2", "p_2", xalign=0.6, yalign=0.5, hover_border="images/posters/hover.png")
...
screen game_menu(title, scroll=None, yinitial=0.0):
    ...
    #textbutton _("Вернуться"):
        #style "return_button" xpos 161 ypos 907

        #action Return()

    imagebutton auto "gui/button/bt_back_%s.png" xpos 161 ypos 890 focus_mask True action Return() hovered [ Play("sound", "audio/bumaga.ogg") ]
...
screen file_slots(title):
    ...
    default page_name_value = FilePageNameInputValue(pattern=_("{} страница"), auto=_(""), quick=_(""))
        ...
        fixed:
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        action FileAction(slot) hovered [ Play("sound", "audio/bumaga.ogg") ]
...
screen preferences():
    ...
    add "gui/preferences_menu.png"

    use navigation
    imagebutton auto "gui/button/bt_back_%s.png" xpos 161 ypos 890 focus_mask True action Return() hovered [ Play("sound", "audio/bumaga.ogg") ]


    text _("Режим экрана") xpos 390 ypos 250

    imagebutton auto "gui/button/full_%s.png" xpos 270 ypos 320 focus_mask True action Preference("display", "fullscreen")
    imagebutton auto "gui/button/window_%s.png" xpos 500 ypos 320 focus_mask True action Preference("display", "window")

    text _("Пропуск") xpos 420 ypos 550

    textbutton _("Всего текста") xpos 300 ypos 600 action Preference("skip", "toggle")
    textbutton _("После выборов") xpos 300 ypos 660 action Preference("after choices", "toggle")
    textbutton _("Переходов") xpos 300 ypos 720 action InvertSelected(Preference("transitions", "toggle"))

    text _("Громкость музыки") xpos 1000 ypos 250
    frame xpos 1000 ypos 300:
        style_group "pref"
        bar value Preference("music volume")

    text _("Громкость звуков") xpos 1000 ypos 370
    frame xpos 1000 ypos 420:
        style_group "pref"
    ...
    text _("Скорость авточтения") xpos 1000 ypos 730
    frame xpos 1000 ypos 780:
        style_group "pref"
        bar value Preference("auto-forward time")

init -2 python:
    #espesificaciones para las barras deslisantes
    style.pref_frame.background = None
    style.pref_slider.left_bar = "gui/bar/right.png"
    style.pref_slider.right_bar = "gui/bar/left.png"
...
screen history():
    ...
    frame:
        ...
        background Frame("gui/history_menu.png")
        xmargin 200
        ymargin 0
        xpadding 10
        ypadding 100
        ...
        imagebutton auto "gui/button/bt_history_back_%s.png" xpos 60 ypos 890 focus_mask True action Return() hovered [ Play("sound", "audio/bumaga.ogg") ]
...
#экран ачивки
screen notifyAchieve(message):

    zorder 100
    style_prefix "notify"

    frame at notifyAchieve_appear:
        text "{size=+10}Новое достижение!{/size}\n[message!tq]"
