label jenya_bad_end:
    ...
    mc "Вот сдашь этот экзамен, и ты станешь на шаг ближе, чтобы легко перепоступить куда ты хочешь."
    ...
    "Теперь сомнений не было – этот амулет и вправду меня защищает. Но, пожалуй, мне нужно перестать защищаться и убегать от проблемы. Пора вернуть этот оберег и разобраться со всем самому."

    #Фон коридора вуза.
    scene bg_uni_morning with dissolve
    #Музыка нагнетающая.
    play music room
    #Параллельно с музыкой шум толпы. - nолько в парке есть
    ...
    "Оказавшись в общежитии, я вихрем бросился к комнате Жени. Но мои поиски оказались безрезультатны. Ни в комнате, ни на кухне, нигде её не было."

    "Посещало чувство, что я опоздал, упустил что-то важное. С тяжестью на сердце я побрел в свою комнату."
    ...
    #Та же картинка, но рядом рука с телефоном показывает время

    "Это не мой билет, а Женин. Она улетает домой. Но почему и когда? Может, я ещё могу догнать её? На билете было указано время: 16:30. Сверив с часами, я понял, что самолет только что взлетел."
