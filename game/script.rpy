...
#scene bg_class onlayer back
#scene jenya normal onlayer front at right2
#scene mc normal onlayer inyourface at left2
...
label start:
    ...
    "Я напел этот мотив себе под нос, но слова с глухим стуком будто провалились куда-то. {w}В бездонное небо, может быть. {w}Или в сугроб."
    ...
    "Ну, как присутствие… Разве может кто-то \"присутствовать\" прямо в небе?"
    ...
    "Я все еще не мог пошевелиться. Мороз будто цепями сковал мое тело, но и девушка не спешила подойти и посмотреть мне в лицо."
    ...
    #Звук будильника. Экран становится светлее.
    #Диалоговое окно становится менее различимым. Последнее, что может разглядеть игрок:
    $ quick_menu = False
    ...
    # Проявляется комната главного героя. Фон сдвинут влево на кровать главного героя. Играет приятная музыка (будильник)
    ...
    mc "Стой! Я не расслышал! Как твое имя? Что это за мелодия?"
    ...
    "Это мой будильник. А ведь все было настолько реальным… Даже не верится, что это сон. И почему я так странно себя чувствую после этого?.."
    ...
    #Звук надевания одежды. После этого на экране появляется спрайт главного героя в повседневной одежде.

    scene bg_room_morning_blur

    show mc normal homea with dissolve
    ...
    #Звук скрипа пружин кровати. Камера передвигается направо на спящего на своей кровати Тимура

    #ЗДЕСЬ НУЖНА ПАНОРАМА!!!
    ...
    #Затемняется экран. Фоновая музыка сливается со звуками улицы.
    ...
    #Спрайт гг в верхней одежде, уставший вид.

    show mc sad winter with dissolve
    ...
    show mc normal winter at right2 with moveoutright
    nvl_narrator "Айти и прочие радости жизни"
    ...
    #Спрайт гг с телефоном в руках
    nvl clear

    play sound city

    show mc sad winter at center with moveoutleft

    "Так, кажется, аудитория 3305, а где это?.. "



    #Спрайт гг исчезает. На фоне изображение университета.

    scene bg_hallway with dissolve

    hide mc sad winter at center
    ...
    #На фоне изображена дверь в аудиторию, с номером сверху 3305.
    ...
    #Звук стука в дверь и последующее её открывание. Экран затемняется. Появляется изображение аудитории со студентами.
